...
from .tasks import index, index_all, purge_not_completed, cancel_task, \
                   purge_expired,clean_stale_projects, publish_event, \
                   open_log, append_log, close_log, harakiri, publish_all_indexed, \
                   publish_all_indexed_failed, create_snapshot, generate_models
from .scheduler import prepare_task_routes, schedule_tasks_from_config, register_tasks_from_config
...
TASKS = [index, index_all, purge_not_completed, cancel_task, purge_expired, clean_stale_projects,
         harakiri, publish_event, open_log, append_log, close_log, publish_all_indexed, 
         publish_all_indexed_failed, create_snapshot, generate_models]


def get_app(config_provider, manager_class, tasks=TASKS):
    cfg = config_provider()
